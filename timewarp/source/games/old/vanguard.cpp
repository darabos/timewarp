...
void VanRadar::Paint(BITMAP *Slate, double Tx, double Ty)
{
	for(int num=0; num<physics->num_items; num++)
	{
		...
		if((abs((int)(Tx - o->x))>InternalZoom)&&(o->id!=ORBIT_ID))	continue;
		...
		xpos=(int)shiftscale(Tx, Slate->w/2.,Scale,o->x);
		ypos=(int)shiftscale(Ty, Slate->w/2.,Scale,o->y);
...
bool Vanguard::handle_key(int k)
{
	...
	case KEY_ZOOM_OUT:
		if(RadarZoom<width) RadarZoom*=(1./.8);
		vradar->setSize(RadarZoom);
		return TRUE;
		break;
...
#define LOAD_FLEET log_file("fleets/all.scf"); fleet.load(NULL, "Fleet"); 
	...
	HumanPlayer1 = create_ship(fleet.ship[i]->id, human_control[0], rand()%(int)width, rand()%(int)height, 0, human_team);
...
//Solar system object code
	...
	for (int j=0; j<NSuns; j+=1)
	{
		...
		add(Centre[j] = new Sun(w, h, StarPics[rand()%Num_Star_Pics],0));
