...
void SuperArena::pick_new_ships() {
	...
	Ship *s = create_ship(fleet->ship[i]->id, human_control[0], width/4 , height*3/4, 0, team[1]);
	...
	if ((log->type == Log::log_net1server) || (log->type == Log::log_net1client)) {
		...
		if (i == -1) i = random() % fleet->size;
		Ship *s = create_ship(fleet->ship[i]->id, human_control[1], width/4 , height/4, PI, team[2]);
		add(s->get_ship_phaser());
	...
	for ( j = 1; j < p[1]; j += 1) {
		...
		if (fleet->size>0) {
			...
			log_int(channel_server, i);
			if (i == -1) i = random() % fleet->size;
			e = create_ship(channel_none, fleet->ship[i]->id, "WussieBot", width/4, height*3/4, random(PI2), team[1]);
			add(e->get_ship_phaser());
		}
	}
	if (KTS==1) {
		Pl = new Planet(width/4+500, height*3/4+500, planetSprite, random()%planetSprite->frames());
		add(Pl);
		s[1] = create_ship(channel_none, "staba", "WussieBot", width/4, height*3/4, 1, team[1]);
		add ( new BlinkyIndicator(s[1], 2) );
		handler = new OrbitHandler(Pl,width/2,height/2,random(PI2), (SpaceLocation *)Pl,
		(SpaceLocation *)s[1], 400, 10.0,0);
		add(s[1]);
		game->add(handler);
	}
	...
	if (t>1) {
		for ( j = 0; j < p[2]; j += 1) {
			...
			if (fleet->size>0) {
				...
				log_int(channel_server, i);
				if (i == -1) i = random() % fleet->size;
				e = create_ship(channel_none, fleet->ship[i]->id, "WussieBot", width/4, height/4, random(PI2), team[2]);
				add(e->get_ship_phaser());
			}
		}
		if (KTS==1) {
			Pl = new Planet(width/4+500, height/4+500, planetSprite, random()%planetSprite->frames());
			add(Pl);
			s[2] = create_ship(channel_none, "staba", "WussieBot", width/4, height*3/4, 1, team[2]);
			add ( new BlinkyIndicator(s[2], 4) );
			handler = new OrbitHandler(Pl,width/2,height/2,random(PI2), (SpaceLocation *)Pl,
			(SpaceLocation *)s[2], 400, 10.0,0);
			add(s[2]);
			game->add(handler);
		}
	}
	...
	if (t>2) {
		for ( j = 0; j < p[3]; j += 1) {
			...
			if (fleet->size>0) {
				...
				log_int(channel_server, i);
				if (i == -1) i = random() % fleet->size;
				e = create_ship(channel_none, fleet->ship[i]->id, "WussieBot", width*3/4, height/4, random(PI2), team[3]);
				add(e->get_ship_phaser());
			}
		}
		if (KTS==1) {
			Pl = new Planet(width*3/4+500, height/4+500, planetSprite, random()%planetSprite->frames());
			add(Pl);
			s[3] = create_ship(channel_none, "staba", "WussieBot", width/4, height*3/4, 1, team[3]);
			add ( new BlinkyIndicator(s[3], 4) );
			handler = new OrbitHandler(Pl,width/2,height/2,random(PI2), (SpaceLocation *)Pl,
			(SpaceLocation *)s[3], 400, 10.0,0);
			add(s[3]);
			game->add(handler);
		}
	}
	...
	if (t>3) {
		for ( j = 0; j < p[4]; j += 1) {
			...
			if (fleet->size>0) {
				...
				log_int(channel_server, i);
				if (i == -1) i = random() % fleet->size;
				e = create_ship(channel_none, fleet->ship[i]->id, "WussieBot", width*3/4, height*3/4, random(PI2), team[4]);
				add(e->get_ship_phaser());
			}
		}
		if (KTS==1) {
			Pl = new Planet(width*3/4+500, height*3/4+500, planetSprite, random()%planetSprite->frames());
			add(Pl);
			s[4] = create_ship(channel_none, "staba", "WussieBot", width/4, height*3/4, 1, team[4]);
			add ( new BlinkyIndicator(s[4], 4) );
			handler = new OrbitHandler(Pl,width/2,height/2,random(PI2), (SpaceLocation *)Pl,
			(SpaceLocation *)s[4], 400, 10.0,0);
			add(s[4]);
			game->add(handler);
		}
	}
	...
	if (t>4) {
		for ( j = 0; j < p[5]; j += 1) {
			...
			if (fleet->size>0) {
				...
				log_int(channel_server, i);
				if (i == -1) i = random() % fleet->size;
				e = create_ship(channel_none, fleet->ship[i]->id, "WussieBot", width/2, height*3/4, random(PI2), team[5]);
				add(e->get_ship_phaser());
			}
		}
		if (KTS==1) {
			Pl = new Planet(width/2+500, height*3/4+500, planetSprite, random()%planetSprite->frames());
			add(Pl);
			s[5] = create_ship(channel_none, "staba", "WussieBot", width/4, height*3/4, 1, team[5]);
			add ( new BlinkyIndicator(s[5], 4) );
			handler = new OrbitHandler(Pl,width/2,height/2,random(PI2), (SpaceLocation *)Pl,
			(SpaceLocation *)s[5], 400, 10.0,0);
			add(s[5]);
			game->add(handler);
		}
	}
	...
	if (t>5) {
		for ( j = 0; j < p[6]; j += 1) {
			...
			if (fleet->size>0) {
				...
				log_int(channel_server, i);
				if (i == -1) i = random() % fleet->size;
				e = create_ship(channel_none, fleet->ship[i]->id, "WussieBot", width/4, height/2, random(PI2), team[6]);
				add(e->get_ship_phaser());
			}
		}
		if (KTS==1) {
			Pl = new Planet(width/4+500, height/2+500, planetSprite, random()%planetSprite->frames());
			add(Pl);
			s[6] = create_ship(channel_none, "staba", "WussieBot", width/4, height*3/4, 1, team[6]);
			add ( new BlinkyIndicator(s[6], 4) );
			handler = new OrbitHandler(Pl,width/2,height/2,random(PI2), (SpaceLocation *)Pl,
			(SpaceLocation *)s[6], 400, 10.0,0);
			add(s[6]);
			game->add(handler);
		}
	}
	...
	if (t>6) {
		for ( j = 0; j < p[7]; j += 1) {
			...
			if (fleet->size>0) {
				...
				log_int(channel_server, i);
				if (i == -1) i = random() % fleet->size;
				e = create_ship(channel_none, fleet->ship[i]->id, "WussieBot", width/2, height/4, random(PI2), team[7]);
				add(e->get_ship_phaser());
			}
		}
		if (KTS==1) {
			Pl = new Planet(width/2+500, height/4+500, planetSprite, random()%planetSprite->frames());
			add(Pl);
			s[7] = create_ship(channel_none, "staba", "WussieBot", width/4, height*3/4, 1, team[7]);
			add ( new BlinkyIndicator(s[7], 4) );
			handler = new OrbitHandler(Pl,width/2,height/2,random(PI2), (SpaceLocation *)Pl,
			(SpaceLocation *)s[7], 400, 10.0,0);
			add(s[7]);
			game->add(handler);
		}
	}
	...
	if (t>7) {
		for ( j = 0; j < p[8]; j += 1) {
			...
			if (fleet->size>0) {
				...
				log_int(channel_server, i);
				if (i == -1) i = random() % fleet->size;
				e = create_ship(channel_none, fleet->ship[i]->id, "WussieBot", width*3/4, height/2, random(PI2), team[8]);
				add(e->get_ship_phaser());
			}
		}
		if (KTS==1) {
			Pl = new Planet(width*3/4+500, height/2+500, planetSprite, random()%planetSprite->frames());
			add(Pl);
			s[8] = create_ship(channel_none, "staba", "WussieBot", width/4, height*3/4, 1, team[8]);
			add ( new BlinkyIndicator(s[8], 4) );
			handler = new OrbitHandler(Pl,width/2,height/2,random(PI2), (SpaceLocation *)Pl,
			(SpaceLocation *)s[8], 400, 10.0,0);
			add(s[8]);
			game->add(handler);
		}
	}
	...
	if (g > 0) {
		Pl = new Planet(width/2, height/2, planetSprite, random()%planetSprite->frames());
		add(Pl);
		s = get_config_int(NULL, "Station", 0);
		if (s > 0) {
			Ship *Station1 = create_ship(channel_none, "staba", "WussieBot", width/2, height/2, 1, station_team);
			handler = new OrbitHandler(Pl,width/2,height/2,random(PI2), (SpaceLocation *)Pl,
			(SpaceLocation *)Station1, 400, 10.0,0);
			add(Station1);
			game->add(handler);
		}
	}
	...
	for (m = 0; m < g; m += 1) {
		Pl = new Planet(random() % (int)width, random() % (int)height, planetSprite, random()%planetSprite->frames());
		add(Pl);
		if (s>0) {
			Ship *Station1 = create_ship(channel_none, "staba", "WussieBot", width/2, height/2, 1, station_team);
			handler = new OrbitHandler(Pl,width/2,height/2,random(PI2), (SpaceLocation *)Pl,
