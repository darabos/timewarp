...
class TheHotSpot : public Missile {
	...
	//int			normalhotspot;		// 1: a small one, 2: a special big one
	//	well, this hardly adds to gameplay of the ship. Instead, I try out the
	//	autofocus.
...
ShpHotSpot::ShpHotSpot(Vector2 opos, double angle, ShipData *data, unsigned int code) 
	...
	weaponHeatingTime   = get_config_float("Weapon", "WeaponHeatingTime", 0);
...
void ShpHotSpot::calculate()
{
	...
	turn_rate = turn_rate_default / (1.0 + 0.5*lens_loc*weaponRange/80.0);
...
void ShpHotSpot::animate(Frame *frame)
{
	...
	data->spriteWeapon->animate(Vlens, sprite_index, frame);

	if ( weaponhs && weaponhs->exists() )
		data->spriteWeapon->animate(Vflame, sprite_index+64, frame);
...
int ShpHotSpot::activate_special() {
	if ( weaponhs && weaponhs->exists() )
	//	weaponhs->normalhotspot = 0;	// it's now an abnormal hotspot, a bigger one
	{
		if ( weaponhs->autofocus )
			weaponhs->autofocus = 0;
		else
		{
			weaponhs->autofocus = 1;

//			batt -= 10;
//			if ( batt < 0 )
//				batt = 0;
			extradrain = special_drain;
		}

		fire_special = 0;
		ship->control->keys &= 1 - keyflag::special;
...
Missile(oship, opos, oangle, ov, odamage, orange, oarmour, oship,osprite) 
{
	...
	damage_time = mother->weaponHeatingTime;	// about 0.5 second interval for damage.
	damage_repeat_time = 0;

	//normalhotspot = 1;		// by default, it's a small hotspot thingy
	autofocus = 0;
...
void TheHotSpot::calculate()
{
	...
	for (q.begin(this, layers, passiveRange); q.current; q.next())
	{
		...
		if ( R > mother->focus_loc )	// does the query search work in a square region ?!
			continue;

		if ( D < wi )					// a bit simplistic ; gross approximation !!
			if ( fabs(R - mother->focus_loc) > wi )	// make sure you CAN hit a ship or asteroid
				blockedsight = 1;
...
void TheHotSpot::inflict_damage(SpaceObject *other)
{
	...
	if ( autofocus )
		...
		damage_repeat_time += 2 * frame_time;	// do damage twice as fast.
