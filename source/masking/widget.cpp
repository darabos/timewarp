...
void MAS::Widget::MsgDraw()			{
	...
	if (canvas) {
		if ((flags & D_ANIMATING) && !animator->Animating()) {
			...
			if (animator->GetBuffer()) {
				Draw(animator->GetBuffer());
...
void MAS::Widget::MsgInitSkin() {	GetParent()->HandleEvent(*this, MSG_INITSKIN);	ClearFlag(D_CHANGEDSKIN);	}
...
void MAS::Widget::SetCursor(int i) const {
	if (parent) {
...
bool MAS::Widget::HasFocus() const { return ((flags & D_GOTFOCUS) ? true : false); }
bool MAS::Widget::HasMouse() const { return ((flags & D_GOTMOUSE) ? true : false); }
...
void MAS::Widget::Hide()			{ flags |= D_HIDDEN; }
...
void MAS::Widget::Redraw()			{ flags |= D_DIRTY; }
...
MAS::Font& MAS::Widget::GetFont(int type) { return skin->GetFont(fontInfo.fnt[type]); }
