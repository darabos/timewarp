...
void MAS::CheckBox::MsgInitSkin() {
	Button::MsgInitSkin();
	for (int i=0; i<4; i++) {
		SetFontColor(skin->fcol[Skin::INFO_CHECKBOX][i], skin->scol[Skin::INFO_CHECKBOX][i], i);
		SetFont(skin->fnt[Skin::INFO_CHECKBOX][i], i);
	}
	SetTextMode(skin->c_face);
	SetBitmap(Skin::CHECKBOX);
	SetFlag(D_TOGGLE);
