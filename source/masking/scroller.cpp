...
MAS::Scroller::Scroller() : MAS::Dialog(), visibleItems(1), orientation(0) {
	Add(back);
	Add(bUp);
	Add(bGrip);
	Add(bDown);
...
void MAS::Scroller::HandleEvent(Widget& obj, int msg, int arg1, int arg2) {
	if (msg == MSG_ACTIVATE) {
		if (obj == bUp) {
			double oldpos = pos;
			Decrement();
		...
		switch (arg1) {
			case KEY_LEFT:	if (orientation == 1) newpos -= 1;	break;
			case KEY_RIGHT:	if (orientation == 1) newpos += 1;	break;
			case KEY_UP:	if (orientation == 0) newpos -= 1;	break;
			case KEY_DOWN:	if (orientation == 0) newpos += 1;	break;
