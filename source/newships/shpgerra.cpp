...
private:
	...
	double getAccel(double step);	//	for non-linear acceleration
	double getBrake(double step);	//  for non-linear braking
...
//	if (!thrust || shift_frame > 0) 
	...
	while (extra_hotspot_frame <= 0)
		extra_hotspot_frame += extra_hotspot_rate; //* fabs(0.99-cur_gear_rel);
