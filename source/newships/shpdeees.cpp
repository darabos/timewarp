...
public:
	...
	virtual int  activate_weapon();     // releases a DeepSpaceWarrior
	virtual int  activate_special();    // kills a member of the crew and damages all in range
	...
	virtual void inflict_damage( SpaceObject* other );  // drains crew
	virtual void calculate();           // animates graphics
	virtual void calculate_hotspots();  // leaves trail instead
	virtual int  crewPanelColor(){ return pallete_color[8]; } //grey
};
...
int DeepSpaceEssence::activate_special()
{
	...
	for( q.begin( this, OBJECT_LAYERS, specialRange ); q.currento; q.next() )
	{
		if( q.currento->isShip() && q.currento != this ){
			...
			if( s->crewPanelColor().g >= 225)//palette_color[2] ){
			{
	...
	for( int i=0; i < specialEffectCount; i++ ){
		...
		double e_angle = random(PI2);//(double)(rand() % 360);
...
void DeepSpaceEssence::inflict_damage( SpaceObject* other )
{
	...
	}else if( !other->isblockingweapons) //>isShot() )
	{
		// mostly against DOGI
		int drainDamage = (int)(extraDrainRate * frame_time);
		residualDamage += extraDrainRate * (double)frame_time - drainDamage;
