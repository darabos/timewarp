...
//	control->keys = control->think();
  ...
  if( docked ){
    ...
    sprite_index = get_index(angle);//(int)(angle / 5.625) + 16;
    sprite_index &= 63;
    just_docked = false;
    mass = docked->mass;
#ifdef TERON_COLLISION_FORWARDING
...
int TeronFighter::activate_weapon(){
  game->add( new Missile( this, size * Vector2(0, 0.5),//0.0, 0.5 * h,
...
//void TeronTurret::handle_damage( SpaceLocation* other ){
{
...
//void TeronBuildPlatform::handle_damage( SpaceLocation* other ){
{
