...
void GerlVirtualship::calculate()
{
	...
	if ( hero && hero->exists() )
	{
		...
		{
			batt -= 1.0 * frame_time / 1000.0;		// costs 1 fuel per second
			if ( batt < 0.0 )
			{
				batt = 0.0;
...
//						WELL, it's not that much fun :) can just as well when hitting the enemy
		...
		if ( morons && morons->exists() )
		{
			...
			{
				batt += 8.0 * frame_time * 1E-3;		// 8 recharge per seconds
				if ( batt > batt_max )
					batt = batt_max;
			}
...
void GerlHero::calculate()
{
	...
	if ( beingbusy && !(boardingship && boardingship->exists()) )	// if it's destroyed !
	{
		attached = 0;
		boardingship = 0;
		leavingship = 0;
...
//				dv.length() > (arate + accel_rate) * frame_time * 2)
	...
	if ( comeaboard && !attached )
	{
				...
				fabs(pos.y - boardingship->pos.y) < 1E-3 )
		{
			...
			lastenemyvel = boardingship->vel;	// initialize this check value
...
//		int forcerelease;
		...
		if (boarding_atot > 1 * PI2)	// staying attached uses up fuel
		{
			mother->batt -= 1;
...
//	double iweaponDrain = 4;
	...
	{
		activateyourweapon = 0;
		mother->damage(boardingship, mother->weaponDamage, 0);		// it's the (virtual) mother that does the damage
	...
	if ( (activateyourspecial && !attached && !leavingship) || movetotip )
	{
		...
		if ( !(moronbrother && moronbrother->exists()) )
		{
			...
			if ( movetotip )
			{
				...
				davel = 20.0 * ANGLE_RATIO;		// max. 20 degree per second
				if ( dda > 0 )
					da =  davel;
				else
					da = -davel;
				...
				if ( R - R1 > dR )	// don't move too fast though ;)
					R = R1 + dR; 
				if ( R - R1 < -dR )
					R = R1 - dR;
				...
				vel = moronbrother->vel;	// match velocity with the moron's ship
...
int GerlMorons::avoid_planet()
{
	...
	double d;
	d = distance((SpaceLocation*) p);	//crash?!
