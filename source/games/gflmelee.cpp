...
void HealthBar::animate(Frame *space)
{
	...
	d1 = int(-0.5 * mother->crew_max * scale);
	d2 = d1 + int(mother->crew_max * scale);

	co1 = corner(mother->pos + Vector2(d1, -mother->size.y * 0.6));
	co2 = corner(mother->pos + Vector2(d2, -mother->size.y * 0.6));

	d = min_delta(co2, co1, map_size * space_zoom).x;
	co2 = co1 + Vector2(d, 0);
	co3 = co1 + Vector2(d * mother->crew / mother->crew_max, 0);
	...
	line(space->surface, ix1, iy1, ix3, iy3, col1);
	line(space->surface, ix3, iy3, ix2, iy2, col2);

	space->add_line(ix1, iy1, ix2, iy2);



	d1 = int(-0.5 * mother->batt_max * scale);
	d2 = d1 + int(mother->batt_max * scale);

	co1 = corner(mother->pos + Vector2(d1, -mother->size.y * 0.6 + 2));
	co2 = corner(mother->pos + Vector2(d2, -mother->size.y * 0.6 + 2));

	d = min_delta(co2, co1, map_size * space_zoom).x;
	co2 = co1 + Vector2(d, 0);
	co3 = co1 + Vector2(d * mother->batt / mother->batt_max, 0);
	...
	col1 = makecol(50, 50, 255);	// blue = batt
	col2 = makecol(255, 0, 0);	// red = empty batt
